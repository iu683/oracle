...
# 基础信息
printf -- "%b%-15s%b %s%b\n" "$green" "主机名:" "$yellow" "$hostname" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "运营商:" "$yellow" "$isp_info" "$re"
printf -- "------------------------\n"

# 系统信息
printf -- "%b%-15s%b %s%b\n" "$green" "系统版本:" "$yellow" "$os_info" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "Linux版本:" "$yellow" "$kernel_version" "$re"
printf -- "------------------------\n"

# CPU信息
printf -- "%b%-15s%b %s%b\n" "$green" "CPU架构:" "$yellow" "$cpu_arch" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "CPU型号:" "$yellow" "$cpu_info" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "CPU核心数:" "$yellow" "$cpu_cores" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "CPU占用:" "$yellow" "$cpu_usage_percent" "$re"
printf -- "------------------------\n"

# 内存与硬盘
printf -- "%b%-15s%b %s%b\n" "$green" "物理内存:" "$yellow" "$mem_info" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "虚拟内存:" "$yellow" "$swap_info" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "硬盘占用:" "$yellow" "$disk_info" "$re"
printf -- "------------------------\n"
...
# 网络算法
printf -- "%b%-15s%b %s %s%b\n" "$green" "网络拥堵算法:" "$yellow" "$congestion_algorithm" "$queue_algorithm" "$re"
printf -- "------------------------\n"

# 公网IP
printf -- "%b%-15s%b %s%b\n" "$green" "公网IPv4:" "$yellow" "$ipv4_address" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "公网IPv6:" "$yellow" "$ipv6_address" "$re"
printf -- "------------------------\n"

# 地理位置与时间
printf -- "%b%-15s%b %s %s%b\n" "$green" "地理位置:" "$yellow" "$country" "$city" "$re"
printf -- "%b%-15s%b %s%b\n" "$green" "系统时间:" "$yellow" "$current_time" "$re"
printf -- "------------------------\n"

# 系统运行时长
printf -- "%b%-15s%b %s%b\n" "$green" "系统运行时长:" "$yellow" "$runtime" "$re"
