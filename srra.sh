...
get_system_dns() {
    local dns=""
    while read -r line; do ns=$(echo $line | awk '{print $2}'); [[ -n "$ns" ]] && dns="${dns:+$dns,}$ns"; done < <(grep -E "^nameserver" /etc/resolv.conf)
...
check_port_open() {
    local host_ip=$1 port=$2
    echo -e "${GREEN}[信息] 检测端口 $port 是否开放...${RESET}"
    tcp_status=$(nc -z -w3 $host_ip $port &>/dev/null && echo -e "${GREEN}开放${RESET}" || echo -e "${RED}未开放${RESET}")
...
configure_snell() {
    ...
    echo -e "${YELLOW}\n是否开启 IPv6 解析？${RESET}"; echo -e "${YELLOW}1. 开启   2. 关闭${RESET}"
    read -p "$(echo -e ${GREEN}(默认: 2)${YELLOW}: ${RESET})" ipv6
    ipv6=${ipv6:-2}; ipv6=$([ "$ipv6" = "1" ] && echo true || echo false)
    echo -e "${YELLOW}\n是否开启 TCP Fast Open？${RESET}"; echo -e "${YELLOW}1. 开启   2. 关闭${RESET}"
...
start_service() { echo -e "${GREEN}[信息] 启动 Snell 服务...${RESET}"; systemctl daemon-reload; systemctl enable snell; systemctl start snell; sleep 1; echo -e "${GREEN}[完成] Snell 已启动${RESET}"; log "Snell 启动"; pause; }
stop_service() { systemctl stop snell && echo -e "${GREEN}Snell 已停止${RESET}" && log "Snell 停止"; pause; }
restart_service() { systemctl restart snell && echo -e "${GREEN}Snell 已重启${RESET}" && log "Snell 重启"; pause; }

install_snell() {
    create_user
    mkdir -p $SNELL_DIR; cd $SNELL_DIR
    ARCH=$(uname -m); VERSION="v5.0.0"
    ...
    chmod +x "$SNELL_DIR/snell-server"; rm -f snell.zip
    configure_snell; create_service; start_service
    log "Snell 已安装并启动"
}

update_snell() { stop_service; install_snell; restart_service; echo -e "${GREEN}[完成] Snell 已更新${RESET}"; log "Snell 已更新"; pause; }
uninstall_snell() { stop_service || true; systemctl disable snell || true; rm -f $SNELL_SERVICE; rm -rf $SNELL_DIR; systemctl daemon-reload; echo -e "${GREEN}[完成] Snell 已卸载${RESET}"; log "Snell 已卸载"; pause; }
